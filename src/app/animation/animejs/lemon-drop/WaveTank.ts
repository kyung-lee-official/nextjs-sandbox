export interface Spring {
	/* p: vertical position */
	p: number;
	/* v: an auxilary factor to simulate spring without trigonometric functions */
	v: number;
}
	...
	update(springs: Spring[]) {
		...
		for (let t = 0; t < 8; t++) {
			for (let i = 0; i < springs.length; i++) {
				const prev: Spring =
					springs[(i - 1 + springs.length) % springs.length];
				const next: Spring = springs[(i + 1) % springs.length];
